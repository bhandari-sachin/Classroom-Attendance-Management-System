...
class TeacherSessionReportHandlerTest {
    ...
    void coverage_only() throws Exception {
        ...
        TeacherSessionReportHandler handler =
                new TeacherSessionReportHandler(null, null, null, null);

        // Minimal HttpExchange stub (implements ALL abstract methods)
        HttpExchange ex = new HttpExchange() {
            ...
            @Override public URI getRequestURI() {
                return URI.create("/teacher/session/report?sessionId=10");
            }
            ...
            @Override public void sendResponseHeaders(int code, long length) {// No-op: attributes not needed for testing
            }
        ...
        BaseHandler.RequestContext ctx = new BaseHandler.RequestContext(null) {
            @Override public Long getLongQuery(String key) { return 10L; }
