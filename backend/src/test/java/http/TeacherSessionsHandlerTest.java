...
class TeacherSessionsHandlerTest {
    ...
    void missingAuthorization_returns403() throws Exception {
        ...
        var ex = new FakeExchange("GET", "/teacher/sessions?classId=1", null);
    ...
    void methodNotAllowed_returns405() throws Exception {
        ...
        var handler = new TeacherSessionsHandler(jwtService, classSQL, sessionSQL);
        var ex = new FakeExchange("PUT", "/teacher/sessions?classId=1", null);
    ...
    void get_missingClassId_returns400() throws Exception {
        ...
        var handler = new TeacherSessionsHandler(jwtService, classSQL, sessionSQL);
        var ex = new FakeExchange("GET", "/teacher/sessions", null);
        ex.getRequestHeaders().set("Authorization", "Bearer token");
    ...
    void get_success_returns200_withData() throws Exception {
        ...
        when(sessionSQL.listForClass(5L)).thenReturn(Collections.emptyList());
    ...
    void post_missingClassId_returns400() throws Exception {
        ...
        var handler = new TeacherSessionsHandler(jwtService, classSQL, sessionSQL);
        var ex = new FakeExchange("POST", "/teacher/sessions", "{\"x\":1}");
        ex.getRequestHeaders().set("Authorization", "Bearer token");
    ...
    void post_forbiddenNotOwner_returns403() throws Exception {
        ...
        var handler = new TeacherSessionsHandler(jwtService, classSQL, sessionSQL);
        var ex = new FakeExchange("POST", "/teacher/sessions", "{\"classId\":5}");
        ex.getRequestHeaders().set("Authorization", "Bearer token");
    ...
    void post_endBeforeStart_returns400() throws Exception {
        ...
        """;
        var ex = new FakeExchange("POST", "/teacher/sessions", json);
        ex.getRequestHeaders().set("Authorization", "Bearer token");
    ...
    void post_success_returns201_andCallsCreateSession() throws Exception {
        ...
        when(classSQL.isClassOwnedByTeacher(5L, 10L)).thenReturn(true);
        when(sessionSQL.createSession(eq(5L), any(LocalDate.class), any(LocalTime.class),
                any(LocalTime.class), anyString()))
                .thenReturn(999L);
        ...
        verify(sessionSQL).createSession(eq(5L), any(LocalDate.class),
                any(LocalTime.class), any(LocalTime.class), anyString());
    }

    // ---------------------------------------------------------
    // ✅ FIXED TEST (ROBUST - no fragile string dependency)
    // ---------------------------------------------------------
    @Test
    void post_invalidJson_returns500() throws Exception {
        ...
        var handler = new TeacherSessionsHandler(jwtService, classSQL, sessionSQL);
        var ex = new FakeExchange("POST", "/teacher/sessions", "{bad-json}");
        ex.getRequestHeaders().set("Authorization", "Bearer token");
        ...
        assertTrue(
                body != null &&
                        (
                                body.toLowerCase().contains("internal") ||
