...
public class AdminPages {
    ...
    public static Parent dashboardPage(Scene scene, AppRouter router, JwtStore jwtStore, AuthState state) {
        ...
        long totalStudents = users == null ? 0 : users.stream().filter(u -> u.getUserType() == UserRole.STUDENT).count();
        long totalTeachers = users == null ? 0 : users.stream().filter(u -> u.getUserType() == UserRole.TEACHER).count();
